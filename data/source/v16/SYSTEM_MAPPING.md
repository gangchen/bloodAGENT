# ISBT V16 系统 ↔ bloodAGENT 数据 — 系统名映射
...
## ISBT V16 全部 48 个血型系统（按 isbt_number 排序）
...
## bloodAGENT `system/gene` 列与 ISBT V16 的命名差异

| bloodAGENT 写法 | ISBT V16 system symbol | 说明 |
|---|---|---|
...
**升级策略建议**：保持 bloodAGENT 现有 `system/gene` 列内容为"gene name"（与代码 `CIsbtGt2Pt` 的索引键一致，破坏成本太高），新增系统按其主基因名（如 PIEZO1 而非 ER）作为 `system/gene` 值。V16 的 `gene.name` 字段即可直接复用。

## 13 个新系统的主基因（用于 bloodAGENT system/gene 列）

| ISBT V16 symbol | 主基因（拟用作 bloodAGENT `system/gene`） | 染色体 |
|---|---|---|
