#!/usr/bin/env python3
"""
Regression analysis V16 vs pre-V16:
  - For each baseline-called allele, check that it still appears in V16's call set.
  - For each baseline phenotype, check that it still appears in V16's phenotype set.
  - Quantify call-set expansion (more alleles tying at top score is a known
    consequence of V16's larger allele table when input VCF can't discriminate).
  - List the new V16 systems that produced calls.

Usage:
  python3 data/source/v16/tools/analyze_regression.py
"""
...
def hap_match(baseline_set: set, v16_set: set) -> bool:
    """True if every baseline call is preserved in V16 (V16 may have MORE alleles)."""
    return bool(baseline_set) and baseline_set.issubset(v16_set)


def analyze(sample: str, baseline_path: Path, v16_path: Path) -> dict:
    base = parse_loci(baseline_path)
    v16 = parse_loci(v16_path)
    if not base or not v16:
        return {"sample": sample, "skip": True, "base_path": str(baseline_path), "v16_path": str(v16_path)}

    common = set(base) & set(v16)
    only_v16 = set(v16) - set(base)
    only_base = set(base) - set(v16)
    allele_preserved = []   # systems where every baseline allele is still in V16's top set
    allele_dropped = []     # systems where V16 LOST a baseline allele
    phen_preserved = []
    phen_dropped = []
    expansion = defaultdict(int)  # how many extra alleles per haplotype V16 introduces

    for s in common:
        # diploid: 2 haplotypes; require BOTH haps to preserve baseline allele
        hap_count = max(len(base[s]["alleles_per_hap"]), len(v16[s]["alleles_per_hap"]))
        if hap_count == 0:
            continue
        a_ok = all(
            hap_match(base[s]["alleles_per_hap"][i], v16[s]["alleles_per_hap"][i])
        ...
        p_ok = all(
            hap_match(base[s]["phens_per_hap"][i], v16[s]["phens_per_hap"][i])
        ...
        for i in range(min(len(base[s]["alleles_per_hap"]), len(v16[s]["alleles_per_hap"]))):
            expansion[s] = max(expansion[s], len(v16[s]["alleles_per_hap"][i]) - len(base[s]["alleles_per_hap"][i]))

    return {
        "sample": sample,
        "common_systems": len(common),
        "only_v16_systems": sorted(only_v16),
...
def main():
    ...
    for s, baseline_name in samples:
        ...
        r = analyze(s, b, v)
        if r.get("skip"):
            print(f"=== {s} === SKIP (missing baseline or V16)")
            continue
        print(f"=== {s} ===")
        print(f"  common systems: {r['common_systems']}")
        print(f"  systems gained in V16: {r['only_v16_systems'] or '-'}")
        print(f"  systems lost in V16:   {r['only_base_systems'] or '-'}")
        print(f"  baseline allele PRESERVED in V16 top set: {r['allele_preserved']}/{r['common_systems']}")
