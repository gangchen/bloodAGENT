#!/usr/bin/env python3
"""
Generate exonic_annotation supplement rows for V16 systems NOT in the existing
...
Source: V16 /api/gene gives chromosome, hg19/hg38 start/end, and exon_count
...
# System -> primary gene name (for new V16 systems)
NEW_SYSTEM_GENES = {
    "CH_RG":   ["C4A", "C4B"],
    "KANNO":   ["PRNP"],
    "SID":     ["B4GALNT2"],
...
def main():
    ...
    ap.add_argument("--raw-dir", default="data/source/v16/raw")
    ap.add_argument("--out-dir", default="data/source/v16/derived")
    args = ap.parse_args()
        ...
        with out.open("w") as f:
            ...
            for sys_sym, gene_list in NEW_SYSTEM_GENES.items():
                for gn in gene_list:
                    ...
                    # Per-exon coords unknown from V16 API. Leave empty; populate from UCSC refGene.
                    cds_start = ""
                    cds_end = ""
                    exon_starts = ""
                    exon_ends = ""
