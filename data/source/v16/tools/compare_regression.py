#!/usr/bin/env python3
"""
Compare V16-run bloodAGENT phenotype JSONs against the pre-V16 baseline JSONs
bundled in data/testdata/. Emits a per-sample diff highlighting allele renames
and phenotype changes.

Usage:
  python3 data/source/v16/tools/compare_regression.py \
    --baseline-dir data/testdata \
    --v16-dir data/source/v16/derived/regression

For each test sample (HGDP00001/00003/00005, NA24143):
  - Match the V16 *.v16.json against the pre-V16 *.phased.json (or *.json)
  - For each system: compare called allele name(s) + phenotype(s)
  - Report:
      [unchanged]  same allele + same phenotype       OK
      [renamed]    different allele name, same phenotype string  -> likely V16 rename
      [phen_diff]  same allele but phenotype string changed
      [new_sys]    system present in V16 but not in baseline (the 12 new systems)
      [lost_sys]   system was in baseline but missing in V16 (should be empty)
"""
...
def diff_sample(baseline: Path, v16: Path):
    print(f"\n=== {baseline.name} vs {v16.name} ===")
    b = parse_loci(baseline) if baseline.exists() else {}
    n = parse_loci(v16) if v16.exists() else {}
    if not b:
        print(f"  (no baseline at {baseline})")
    if not n:
        print(f"  (no V16 at {v16} — skip)")
        return
...
def main():
    ...
    for s, baseline_name in samples:
        ...
        v16 = Path(args.v16_dir) / f"{s}.v16.json"
        diff_sample(baseline, v16)
