#!/usr/bin/env python3
"""
Backport the `strand (hg19)` and `strand (hg38)` columns into the V16-derived
variation_annotation.v16.dat, using two sources:
  ...
  2. A small hand-curated dictionary of RefSeq-known strands for genes that
     don't appear in #1 (i.e., the 12 new V16 systems).

After this, the V16 vanno has every row's strand populated (was '+' default
...
# Hand-curated strand for V16 genes NOT present in the old master.
# Source: NCBI Gene / RefSeq (verified against the corresponding hg38 entries).
EXTRA_STRAND = {
    # New ISBT V16 systems (12 missing)
    "C4A":     "+",   # chr6, C4A
    "C4B":     "+",   # chr6, C4B (CH/RG)
    "PRNP":    "+",   # chr20, KANNO
    "B4GALNT2":"-",   # chr17, SID
    ...
    "ATP11C":  "-",   # chrX,  ATP11C
    "MAL":     "-",   # chr2,  MAL
    # Other gene-name keys that appear in V16 but not in old master
...
def main():
    ...
    ap.add_argument("--vanno", required=True, help="V16-derived variation_annotation.v16.dat to update IN-PLACE")
    ...
    # ISBT V16 uses gene names; some old keys are system symbols. Build alias map.
