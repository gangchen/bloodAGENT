#!/usr/bin/env bash
# Run bloodAGENT (V16 image) against the 4 bundled test samples and write
# the V16 phenotype JSONs into data/source/v16/derived/regression/.
#
# Usage:
#   data/source/v16/tools/run_regression.sh [docker_image]
#
# Default image: bloodagent:v16
set -eo pipefail

IMG="${1:-bloodagent:v16}"
HERE="$(cd "$(dirname "$0")" && pwd)"
ROOT="$(cd "${HERE}/../../../.." && pwd)"
OUT="${ROOT}/data/source/v16/derived/regression"
mkdir -p "${OUT}"
...
run_sample() {
  ...
  docker run --rm \
    ...
    "${IMG}" \
      ...
      --out "/work/data/source/v16/derived/regression/${sample}.v16.json" \
