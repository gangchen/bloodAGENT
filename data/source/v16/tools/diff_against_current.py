#!/usr/bin/env python3
"""
Diff the V16-derived gt2pt against the current bloodAGENT gt2pt to surface
added / renamed / retired alleles and per-system delta counts.

Output (in --out-dir):
  - allele_diff.tsv: allele, status (added/retired/unchanged/renamed?), v16_system, current_system, v16_phen, current_phen
  - system_summary.tsv: system, current_count, v16_count, added, retired
"""
...
def main():
    ...
    ap.add_argument("--v16", required=True, help="Path to V16-derived .dat (e.g. derived/genotype_to_phenotype_annotation.v16.dat)")
    ap.add_argument("--out-dir", default="derived")
    args = ap.parse_args()

    cur = read_gt2pt(Path(args.current))
    new = read_gt2pt(Path(args.v16))
    ...
    with (out / "allele_diff.tsv").open("w") as f:
        ...
        w.writerow(["allele", "status", "v16_system", "current_system", "v16_phenotype", "current_phenotype"])
    ...
    with (out / "system_summary.tsv").open("w") as f:
        w = csv.writer(f, delimiter="\t")
        w.writerow(["system", "current_count", "v16_count", "delta"])
        for s in all_sys:
